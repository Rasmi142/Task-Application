...
// creating cards in js to display on ui
// writing html code in js file because of reuse this function for muktiple times in ui
  ...
  const htmlModalContent = ({ id, title, description, url }) => {
    ...
    <div id=${id}>
     ${
       url
         ? `<img width="100%" src=${url} alt="card image cap" class="img-fluid md-3 rounded-lg"/>`
  ...
  const saveEdit = (e) => {
    ...
    const updateData = {
      ...
      tags: taskDescription.innerHTML
    }
  ...
  //     taskContents.remove(taskContents.firstChild)
  //   }
  ...
  //     taskContents.insertAdjacentElement("beforeend", htmlTaskContent(card))
  //   })
  // }
  ...
  const searchTask = (event) => {
    ...
    const searchText = event.target.value.toLowerCase(); // Convert search text to lowercase for case-insensitive search
  
    const resultData = state.taskList.filter(({ title }) => title.toLowerCase().includes(searchText));
  
    console.log(resultData);
  
    resultData.forEach((cardDate) => {
      taskContents.insertAdjacentHTML("beforeend", htmlTaskContent(cardDate));
    });
  };
